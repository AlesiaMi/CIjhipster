...
import { sampleWithRequiredData } from '../data-source.test-samples';
...
describe('DataSource Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 1785 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.dataSources()[0]).toEqual(expect.objectContaining({ id: 1785 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 1785 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    successReq.flush([{ id: 1785 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to dataSourceService', () => {
      ...
      vitest.spyOn(service, 'getDataSourceIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.dataSourcesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.dataSourcesParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
