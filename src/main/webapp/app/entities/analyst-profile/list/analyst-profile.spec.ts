...
import { sampleWithRequiredData } from '../analyst-profile.test-samples';
import { AnalystProfileService } from '../service/analyst-profile.service';

import { AnalystProfile } from './analyst-profile';

vitest.useFakeTimers();

describe('AnalystProfile Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: AnalystProfile;
  let fixture: ComponentFixture<AnalystProfile>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(AnalystProfile);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 27303 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.analystProfiles()[0]).toEqual(expect.objectContaining({ id: 27303 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 27303 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.analystProfiles()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 27303 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to analystProfileService', () => {
      ...
      vitest.spyOn(service, 'getAnalystProfileIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAnalystProfileIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.analystProfilesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.analystProfilesParams()).toMatchObject(
