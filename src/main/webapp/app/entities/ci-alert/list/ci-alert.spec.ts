...
import { sampleWithRequiredData } from '../ci-alert.test-samples';
import { CiAlertService } from '../service/ci-alert.service';

import { CiAlert } from './ci-alert';

vitest.useFakeTimers();

describe('CiAlert Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: CiAlert;
  let fixture: ComponentFixture<CiAlert>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CiAlert);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 1978 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.ciAlerts()[0]).toEqual(expect.objectContaining({ id: 1978 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 1978 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.ciAlerts()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 1978 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to ciAlertService', () => {
      const entity = { id: 1978 };
      vitest.spyOn(service, 'getCiAlertIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCiAlertIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.ciAlertsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.ciAlertsParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
