...
import { sampleWithRequiredData } from '../analysis-result.test-samples';
...
describe('AnalysisResult Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: AnalysisResult;
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 28488 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.analysisResults()[0]).toEqual(expect.objectContaining({ id: 28488 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 28488 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.analysisResults()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 28488 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to analysisResultService', () => {
      ...
      vitest.spyOn(service, 'getAnalysisResultIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAnalysisResultIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.analysisResultsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.analysisResultsParams()).toMatchObject(
